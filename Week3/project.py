...
def fast_closest_pair(cluster_list):
    ...
    def fast_helper(cluster_list, horiz_order, vert_order):
        ...
        def _div(horiz_order):
            """
            divide
            """
            return int(math.ceil(len(horiz_order) / 2.0))

        # base case
        if len(horiz_order) <= 3:
            sublist = [cluster_list[horiz_order[i]]
                    for i in range(len(horiz_order))]
            res = list(slow_closest_pairs(sublist))[0]
            return res[0], horiz_order[res[1]], horiz_order[res[2]]

        # divide
        mid = 0.5 * (cluster_list[horiz_order[_div(horiz_order) - 1]].horiz_center() +
                    cluster_list[horiz_order[_div(horiz_order)]].horiz_center())

        _hlr = horiz_order[0: _div(horiz_order)], horiz_order[_div(horiz_order): len(horiz_order)]
        min_d = min(fast_helper(cluster_list, _hlr[0],
            [vi for vi in vert_order if vi in frozenset(_hlr[0])]),
            fast_helper(cluster_list, _hlr[1],
                [vi for vi in vert_order if vi in frozenset(_hlr[1])]))
        
        # conquer
        sss = [vi for vi in vert_order if
                abs(cluster_list[vi].horiz_center() - mid) < min_d[0]]
